...
export type BrainlinkLawScope = 'GLOBAL' | 'ORGANIZATION' | 'PROJECT' | 'WORKER';
export type BrainlinkLawTrigger = 'ALWAYS' | 'SESSION_START' | 'TASK_START' | 'TASK_END' | 'ON_ERROR';

export interface BrainlinkLaw {
  id: string;
  title: string;
  scope: BrainlinkLawScope;
  targetId?: string;
  trigger: BrainlinkLawTrigger;
  version: number;
  body: string;
  mandatory: boolean;
  enabled: boolean;
