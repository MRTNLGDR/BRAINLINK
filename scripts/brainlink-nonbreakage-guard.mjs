...
check('v2.2 metadata remains NOT_PROMOTED', lock.includes('brainlink_candidate_release=v2.2') && lock.includes('brainlink_candidate_status=NOT_PROMOTED'));
...
check('Stable runtime manifest excludes v2.2 execution files', !stableManifest.includes('execution.ts') && !stableManifest.includes('execution.spec.ts'));
...
console.log(`\n${13 - failures.length}/13 non-breakage checks passed.`);
