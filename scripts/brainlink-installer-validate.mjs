...
const workspaceShell = read('scripts/apply-brainlink-workspace-shell.mjs');
const organizerTransform = read('scripts/apply-brainlink-organizer.mjs');
const organizerRuntime = read('brainlink-runtime/organizer/brainlink-organizer.tsx');
...
check('Brainlink uses one native workspace shell', materialize.includes("import { applyBrainlinkWorkspaceShell } from './apply-brainlink-workspace-shell.mjs';") && materialize.includes('applyBrainlinkWorkspaceShell({ targetRoot: target });') && workspaceShell.includes('data-ui="brainlink-workspace-native"') && workspaceShell.includes("path: '/brainlink/*'") && workspaceShell.includes('const BrainlinkButton = () =>') && workspaceShell.includes('navigateInWorkspace={target => workbench.open(target)}') && workspaceShell.includes('navigatePath(result.path)') && !workspaceShell.includes("navigate('/brainlink/governance')"));
check('Brainlink organizes life, ideas, company, people, budget and Brains', materialize.includes("import { applyBrainlinkOrganizer } from './apply-brainlink-organizer.mjs';") && materialize.includes('applyBrainlinkOrganizer({ sourceRoot, targetRoot: target });') && organizerTransform.includes("slug: 'organization'") && organizerTransform.includes("slug: 'brains'") && organizerRuntime.includes('WorkspaceService') && organizerRuntime.includes('Documentation compliance') && organizerRuntime.includes('brainlink:organizer:v1') && workspaceShell.includes("{ label: 'Brains', slug: 'brains' }") && workspaceShell.includes('>Brain</span>'));
