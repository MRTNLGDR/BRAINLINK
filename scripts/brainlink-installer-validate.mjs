...
check('Production path builds a self-contained local AFFiNE web package', packagePatch.includes("BRAINLINK_BUILD_SCRIPT = 'cross-env PUBLIC_PATH=/ yarn affine web build && node scripts/brainlink-verify-web-build.mjs'") && packagePatch.includes("'brainlink:build': BRAINLINK_BUILD_SCRIPT") && exists('.brainlink-runtime-overrides/scripts/brainlink-verify-web-build.mjs') && oneClick.includes('runBuildWithRecovery'));
