...
const governanceBridge = read('scripts/apply-affine-governance-bridge.mjs');
const governanceStore = read('scripts/brainlink-governance-store.mjs');
const governancePanel = read('brainlink-runtime/governance/governance-panel.tsx');
const brandUnification = read('scripts/apply-brainlink-brand-unification.mjs');
const brainlinkBrand = read('brainlink-runtime/branding/brainlink-brand.ts');
...
check('Governance bridge is durable, live and preserves project routing', materialize.includes('applyAffineGovernanceBridge') && governanceBridge.includes('governance-source.json') && governanceBridge.includes('const renderProjects = () =>') && governanceBridge.includes('nextRenderer') && governancePanel.includes('@tanstack/react-query') && governanceStore.includes('/api/governance/snapshot') && governanceStore.includes('Cache-Control') && governancePanel.includes('refetchInterval: 15_000') && governancePanel.includes('GOVERNANCE_UPDATED_EVENT') && !governancePanel.includes('fallbackData'));
check('Brainlink is the single visible product identity', materialize.indexOf('applyBrainlinkBrandUnification') > materialize.indexOf('applyAffineLocalAi') && brandUnification.includes("brainlink/brainlink-brand") && brandUnification.includes("replaceAll('AFFiNE', 'Brainlink')") && brainlinkBrand.includes('MutationObserver') && brainlinkBrand.includes('document.title') && brainlinkBrand.includes('ATTRIBUTE_NAMES'));
