...
MANIFEST="$ROOT/BRAINLINK_ZIP_CANDIDATE_V23_RUNTIME.sha256"
...
[[ -f "$MANIFEST" ]] || { echo 'Compact v2.3 runtime manifest is missing' >&2; exit 1; }
...
MANIFEST_SHA="$(read_lock candidate_runtime_manifest_sha256)"
[[ "$ARCHIVE_SHA" =~ ^[0-9a-f]{64}$ ]] || { echo 'candidate_runtime_overlay_sha256 is not pinned' >&2; exit 1; }
[[ "$MANIFEST_SHA" =~ ^[0-9a-f]{64}$ ]] || { echo 'candidate_runtime_manifest_sha256 is not pinned' >&2; exit 1; }
...
echo '[BRAINLINK] Applying ZIP-authoritative compact runtime over stable baseline...'
...
echo '[BRAINLINK] ZIP-authoritative candidate v2.3 compact runtime materialized and verified.'
